...
def download_data(symbol):
    ...
    for single_date in df["time"].dt.date.unique():
        ...
        base_dir = f"data/historical_kline/{symbol}/{year}/{month}/{day}/{INTERVAL}"
...
def main():
    ...
    output_path = os.path.join(output_dir, "volatile_symbols.txt")

    with open(output_path, "w", encoding="utf-8") as f:
        from config_scan_dashboard import  SCAN_FROM,SCAN_TO
        f.write(f"🗓️ טווח סריקה: {SCAN_FROM} עד {SCAN_TO}\n")
        f.write(f"📊 נמצאו {len(volatiles)} מטבעות תנודתיים\n\n")
        f.write("\n".join(volatiles))

    print(f"📝 נשמר לקובץ: {output_path}")
